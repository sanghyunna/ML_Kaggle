...
def predict(df, ss, lgb, lgb_two=False):
    # df = df.astype(int)
    df = preprocess(df.copy())
    df_transformed = df.copy()
    ss.fit(df)
    df_transformed = ss.transform(df)
    predictions = lgb.predict(df_transformed)
...
def predict_and_save(ss, lgb, lgb_two=False):
    test_df = pd.read_csv('test.csv')
    test_target = predict(test_df, ss, lgb, lgb_two)  
...
#     print(f"{titanic_input.columns[i]}: {lgb.feature_importances_[i]}")

# print("Train accuracy:", lgb.score(titanic_input_scaled, titanic_target))
