...
def preprocess(df, isTest=False):
    ...
    if not isTest:
        ...
        median_saleprice_dictionary['MiscFeatureSalePriceMedian'] = median_miscfeature_saleprice
    else:
        median_miscfeature_saleprice = median_saleprice_dictionary['MiscFeatureSalePriceMedian']
...
lgb = LGBMRegressor(verbose=-1, device='gpu', gpu_platform_id=1, gpu_device_id=0)

iteration = 1000

rs = RandomizedSearchCV(lgb, params, n_iter=iteration, cv=5, n_jobs=-1)

start = time.perf_counter()

rs.fit(train_scaled, saleprice_target)

best_lgb = rs.best_estimator_

train_predictions = best_lgb.predict(train_scaled)

end = time.perf_counter()

print(f"Time: {end - start:0.4f} seconds for {iteration} iteration")
...
testdf_predictions = best_lgb.predict(testdf_scaled)
