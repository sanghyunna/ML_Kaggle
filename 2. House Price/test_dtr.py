...
dt = DecisionTreeRegressor() # max_depth를 지정하지 않으면 28까지 학습함
dt.fit(train_scaled, saleprice_target)

train_predictions = dt.predict(train_scaled)
...
testdf_predictions = dt.predict(testdf_scaled)

results = pd.DataFrame({'Id': test_df['Id'], 'SalePrice': testdf_predictions})
results.to_csv('./submission_dt_28.csv', index=False) # 매우 과대적합이지만, 테스트 점수 역시 증가하였으므로 제출해봄
