...
random = np.random.randint(0, 1000, no_of_random_values)
# 하이퍼파라미터 확인을 위한 루프
...
for lasso_iteration in iteration:
    print(f"iteration: {lasso_iteration} ", end="")
    # 0~1000 사이 랜덤값 생성
    
    train_result = []
    test_result = []

    # for random_value in random:
    for random_value in range(0,100):
        # print(f"[{random_value}] ", end="")
        train_input, test_input, train_target, test_target = train_test_split(saleprice_input, saleprice_target, random_state=random_value)
        ...
        lasso = Lasso(alpha=0.001, max_iter=lasso_iteration)
    ...
    print("train: ", end="")
    print(np.mean(train_result), end=", ")
    print("test: ", end="")
    print(np.mean(test_result))
    train_mean_result.append(np.mean(train_result))
    test_mean_result.append(np.mean(test_result))

    train_result = []
    test_result = []

# import matplotlib.pyplot as plt
# plt.plot(iteration, train_mean_result, label="train")
# plt.plot(iteration, test_mean_result, label="test")
# plt.legend()
# plt.show()
...
#     print(f"{saleprice_input.columns[i]}: {lasso.coef_[i]}")